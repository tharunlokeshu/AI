  ...
  async function scrapeGoogleMaps(location) {
    ...
    try {
      ...
      for (let i = 0; i < Math.min(results.length, 10); i++) {
        ...
        let address = 'N/A';
        try {
          address = await result.$eval('.section-result-location, .fontBodyMedium, .hfpxzc, [data-item-id*="address"]', el => el.textContent.trim()) ||
        ...
        let phone = '';
        try {
          phone = await result.$eval('.section-result-phone-number, [data-tooltip*="Call"], .rllt__details, [data-item-id*="phone"]', el => el.textContent.trim()) ||
                 await result.$eval('[aria-label*="phone"], [aria-label*="call"]', el => el.textContent.trim()) ||
                 '';
        } catch (e) {
          phone = '';
        }
